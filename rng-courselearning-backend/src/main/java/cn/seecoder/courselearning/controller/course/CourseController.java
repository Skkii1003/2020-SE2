...
public class CourseController {
    ...
    public CourseVO getCourseByPrimaryKey(@PathVariable Integer courseId){
        return new CourseVO(courseService.getByPrimaryKey(courseId));
    }
    /**
     * 查询单一课程
     * @param uid 用户id
    ...
    public ResultVO<CourseVO> setCourseLike(@RequestParam Integer uid, @PathVariable Integer courseId) {
        if(!courseService.getCourseLike(uid, courseId).getData().isLiked())
            return courseService.addCourseLike(uid,courseId);
        return null;
    }
